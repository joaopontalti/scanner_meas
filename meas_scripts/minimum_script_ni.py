...
name = 'testing_NI_gustavo'
main_folder = 'C:/Users/joaop/anaconda3/envs/Dataset_JoaoP/TCC_DATASET_JoaoP/DATA2025'# use forward slash
# arduino_dict = dict()
source = Source(coord = [0, 0, 0.3])
#%%
meas_obj = ScannerMeasurement(main_folder = main_folder, name = name,
    fs = 51200, fft_degree = 18, start_stop_margin = [0.1, 0.4], 
    mic_sens = 52.8, x_pwm_pin = 2, x_digital_pin = 24,
...
#%%
meas_obj.ni_initializer(buffer_size = 2**10, repetitions = 1)
meas_obj.ni_set_output_channels(physical_channel_nums=[0, 2], 
                                out_channel_to_ni = 0, out_channel_to_amp = 2, 
                                ao_range = 10.0)

meas_obj.ni_set_input_channels(in_channel_ref_onrec = 0, in_channel_sensor_onrec = 1,
                           ai_range = 1, sensor_sens = 52.8, sensor_current = 4e-3)
...
meas_obj.set_motors()
#%%
meas_obj.sequential_measurement(meas_with_ni = True)


#%% 
meas_obj.set_motors()
meas_obj.move_motor('z', 0.01)
